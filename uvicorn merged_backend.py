...
async def load_models():
    ...
    try:
        ...
        raise e  # Fail startup if models can't load
...
app = FastAPI(
    title="Professional AI Toolkit (Python 3.10+)",
...
def get_status():
    """
    Returns detailed status of the AI models and system.
    """
    return {
...
def answer_question(payload: QuestionAnsweringIn):
    ...
    try:
        ...
        return QuestionAnsweringOut(
            answer=answer_text,
            confidence=0.85,
            sources=["Mock Source: Wikipedia", "Mock Source: Research Paper"]
        )
    except Exception as e:
...
# Add root health check for Hugging Face Spaces
@app.get("/health")
def root_health_check():
    """
    Root health check endpoint for Hugging Face Spaces.
    """
    return {"status": "healthy", "models_loaded": MODELS_LOADED}
