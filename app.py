...
# --- Helper Functions ---
# Removed get_backend_status as the /status endpoint is removed from backend
# @st.cache_data(ttl=3)
    ...
    if st.button("🔍 Analyze Sentiment", key="sentiment_button", use_container_width=True):
        ...
        if text_input.strip():
            ...
            try:
                ...
                response = requests.post(f"{API_BASE_URL}/api/sentiment/analyze", json={"text": text_input})
                response.raise_for_status()
...
def text_summarization_component():
    ...
    if st.button("📝 Generate Summary", key="summarization_button", use_container_width=True):
        ...
        try:
            ...
            response = requests.post(f"{API_BASE_URL}/api/summarization/summarize", json={"text": processed_text})
            response.raise_for_status()
            result = response.json()
    ...
    with col2:
        ...
        if st.button("🚀 Generate Content", key="generation_button", use_container_width=True):
            ...
            try:
                ...
                response = requests.post(f"{API_BASE_URL}/api/generation/generate", json={"text": text_input})
                response.raise_for_status()
                result = response.json()
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Analyze Image", key="captioning_button", use_container_width=True):
            ...
            try:
                ...
                response = requests.post(f"{API_BASE_URL}/api/image/caption", files=files)
                response.raise_for_status()
                result = response.json()
...
def translation_component():
        ...
        try:
            ...
            if uploaded_file:
                ...
                if source_lang == "English" and target_lang == "French":
                    response = requests.post(f"{API_BASE_URL}/api/translation/translate", json={"text": original_text_to_translate})
                    response.raise_for_status()
    ...
    def question_answering_component():
        ...
        if st.button("🤔 Get Answer", key="qa_button", use_container_width=True):
            ...
            try:
                # Call FastAPI backend for QA
                response = requests.post(f"{API_BASE_URL}/api/qa/answer", json={"question": question_input, "context": context_input})
                response.raise_for_status()
                result = response.json()
                ...
                if confidence_display:
                    ...
                    with col3:
                        st.metric("Processing Time", "2.1s") # Mock value
                    with col4:
...
"""
                ...
                display_error(f"Could not get answer: {e}")
    ...
    def speech_to_text_component():
        ...
        if st.button("🔊 Convert to Text", key="stt_convert_button", use_container_width=True):
            ...
            try:
                ...
                response = requests.post(f"{API_BASE_URL}/api/stt", files=files)
                response.raise_for_status()
                result = response.json()
...
[00:45] and converted it to text with high accuracy."""
    ...
    def text_to_speech_component():
        ...
        if st.button("🎤 Generate Speech", key="tts_generate_button", use_container_width=True):
            ...
            try:
                ...
                response = requests.post(f"{API_BASE_URL}/api/tts", json={"text": text_input})
                response.raise_for_status()
    ...
    # Removed backend status check as the /status endpoint is removed
    ...
    # Placeholder for backend status, always assume ready if no status endpoint
    st.sidebar.success("Backend Status: ● Ready (Status endpoint removed)")
    st.sidebar.info("Please ensure the backend server is running on Hugging Face Spaces.")
    st.sidebar.markdown("Backend URL: `https://huggingface.co/spaces/adarshdivase/ai-toolkit-backend`")
    ...
    with tab_settings:
        ...
        current_api_base_input = API_BASE_URL
        new_api_base_from_input = st.text_input("Backend API Base URL:", value=current_api_base_input, key="settings_api_base_input")
        
        if new_api_base_from_input != API_BASE_URL:
            st.warning(f"API Base URL changed from {API_BASE_URL} to {new_api_base_from_input}. This change will apply on next rerun.")
            globals()['API_BASE_URL'] = new_api_base_from_input
