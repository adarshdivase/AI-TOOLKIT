...
app = FastAPI(
    title="Professional AI Toolkit (Python 3.12+)",
...
def load_models():
    ...
    try:
        ...
        logging.info("Language Translation model loaded.")
...
async def text_to_speech(payload: TextIn):
    ...
    try:
        ...
        # The TTS pipeline returns a dictionary with 'audio' (numpy array) and 'sampling_rate'
        ...
        buffer = io.BytesIO()
        # Ensure the audio array is float32, which is standard for soundfile
...
async def speech_to_text(file: UploadFile = File(...)):
    ...
    """
    # Accept common audio types. Streamlit's audio_recorder typically outputs WAV.
    ...
    try:
        ...
        # Load audio data using soundfile to ensure correct format for the model
        # The ASR model expects a certain sampling rate (e.g., 16kHz for Whisper-tiny)
        # We need to ensure the audio is resampled if necessary, but for simplicity,
        # we'll assume the input audio is compatible or the model handles resampling internally.
        # For more robust handling, use librosa.load and resample.
        audio_data, current_sampling_rate = sf.read(buffer)

        # If the model expects a specific sampling rate, you might need to resample:
        # from librosa.core import resample
        # target_sampling_rate = 16000 # Example for Whisper
        # if current_sampling_rate != target_sampling_rate:
        #     audio_data = resample(audio_data, orig_sr=current_sampling_rate, target_sr=target_sampling_rate)

        # The ASR pipeline expects a dictionary with "sampling_rate" and "raw" audio data
